...
#include "HoLTabListWidgetBase.generated.h"

class UCommonButtonBase;

/*
*	struct for the Tab Descriptor
*/
USTRUCT(BlueprintType)
struct FHoLTabDescriptor {
	GENERATED_BODY()
public:
	FHoLTabDescriptor() : bHidden(false), CreatedTabContentWidget(nullptr) {}
...
class UHoLTabButtonInterface : public UInterface {
	GENERATED_BODY()
};

class IHoLTabButtonInterface{
	GENERATED_BODY()

public:
	UFUNCTION(BlueprintNativeEvent, Category = "Tab Button")
	void SetTabLabalInfo(const FHoLTabDescriptor& TabDescriptor);
};
...
class HEROESOFLOST_API UHoLTabListWidgetBase : public UCommonTabListWidgetBase
{
	GENERATED_BODY()
public:
	UFUNCTION(BlueprintCallable, BlueprintPure, Category="Tab List")
	bool GetPreregisteredTabInfo(const FName TabNameId, FHoLTabDescriptor& OutTabInfo);

	// Helper method to get at all the preregisted tab infos
	const TArray<FHoLTabDescriptor>& GetAllPreregisteredTabInfos() { return PreregisteredTabInfoArray; }
	...
	UFUNCTION(BlueprintCallable, Category = "Tab List")
	bool RegisterDynamicTab(const FHoLTabDescriptor& TabDescriptor);
	...
	DECLARE_EVENT_TwoParams(UHoLTabListWidgetBase, FOnTabContentCreatedNative, FName, UCommonUserWidget*);
...
private:
	...
	TArray<FHoLTabDescriptor> PreregisteredTabInfoArray;
	...
	TMap<FName, FHoLTabDescriptor> PendingTabLabelInfoMap;
